...
const SeasonPreviewModal: React.FC<SeasonPreviewModalProps> = ({ competitionName, teams, onClose, onPlayerClick, onTeamClick }) => {
    ...
    const transferStats = useMemo(() => {
        ...
        teams.forEach(t => {
            t.transferHistory.filter(h => h.type === 'BOUGHT').forEach(h => {
                incomingCount++;
                const price = parseFloat(h.price.replace(/[^0-9.]/g, '')) || 0;
                totalSpent += price;
            });
            // Snapshot of current squad foreign ratio
            t.players.forEach(p => {
        ...
        foreignRatio = totalPlayers > 0 ? Math.round((foreignCount / totalPlayers) * 100) : 0;
                                ...
                                <div className="space-y-4">
                                    <div className="flex items-center justify-between">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                ...
                                                <div className="text-[10px] text-slate-400 uppercase font-bold">Biten Transfer</div>
                                            </div>
                                        </div>
                                    </div>
                                    <div className="flex items-center justify-between">
                                    ...
                                    <div className="flex items-center justify-between">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                <div className="text-xl font-black text-white">€{transferStats.totalSpent.toFixed(2)}MR</div>
