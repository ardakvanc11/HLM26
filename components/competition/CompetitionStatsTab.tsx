...
const CompetitionStatsTab: React.FC<CompetitionStatsTabProps> = ({ teams, fixtures, competitionId, onPlayerClick, onTeamClick, currentWeek }) => {
    ...
    const [showInjuryModal, setShowInjuryModal] = useState(false); // NEW: Injury Modal State
    ...
    // Player Modal Internal State (YENİ)
    ...
    ).sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime()), [fixtures, competitionId]);
    ...
    const teamStats = useMemo(() => {
        ...
        compFixtures.forEach(f => {
            ...
            if (h && s) {
                ...
                // Simulate Attendance (Consistent seed would be better but this matches existing logic style in this file)
                // We use a pseudo-random seed based on fixture ID to keep it consistent across renders if possible
                ...
                // Derived Defensive Stats (Simulated based on possession/opp shots)
            ...
            if (a && s) {
                ...
                // Derived Defensive Stats (Simulated)
        ...
        const list = Array.from(statsMap.values()).filter(t => t.gamesPlayed > 0 || t.squadValue > 0);
        
        return list.map(t => {
            // ... (Streak & Form Logic remains same)
    ...
    // --- INJURED PLAYERS CALCULATION (NEW) ---
            ...
            const processRatings = (ratings: any[], teamId: string, conceded: number, isHome: boolean) => {
                ...
                ratings.forEach(r => {
                    ...
                    if (entry) {
                        ...
                        // Calculate Minutes Played for this match
                        let mins = 90;
                        ...
                        // If subbed in then out or red card, handle min
                        ...
                        // Check Starts: If not subbed in
                        const wasSubbedIn = !!subIn;
                        if (!wasSubbedIn) entry.starts++;

                        // Update goalsConceded for EVERYONE in the team who played
                        entry.goalsConceded += conceded;

                        if (r.position === 'GK') {
                             if (conceded === 0) entry.cleanSheets++;
                             // Simulate Saves based on rating and conceded
                        ...
                        // Simulate Shots on Target relative to goals and total shots
                        ...
                        // Simulated Stats for General Tab
                        ...
                        // NEW DEFENSIVE STATS SIMULATION
