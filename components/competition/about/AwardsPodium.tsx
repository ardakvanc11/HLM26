...
// Geçmiş Kazananlar - Şimdilik Boş (Yeni oyun başladığında geçmiş veri olmaz veya veritabanından çekilir)
const PAST_WINNERS: { year: string, name: string, team: string, rating: string }[] = [];
                            ...
                            <div className="bg-[#1f1f1f] rounded-xl border border-[#333] overflow-hidden">
                                {PAST_WINNERS.map((winner, idx) => (
                                    <div key={idx} className="flex justify-between items-center p-3 border-b border-[#333] last:border-0 hover:bg-[#252525] transition-colors">
                                        ...
                                        <div className="flex items-center gap-3">
                                            <span className="text-slate-500 text-xs hidden sm:block">{winner.team}</span>
